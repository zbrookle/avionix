...
class JSONSchemaProps(HelmYaml):
    ...
    :param format: format is an OpenAPI v3 format string. Unknown formats are ignored.
        ...
        - ssn: a U.S. social security number following the regex \
        ^\d{3}[- ]?\d{2}[-]?\d{4}$
        ...
        - duration: a duration string like "22 ns" as parsed by Golang time. \
        ParseDuration or compatible with Scala duration format
    ...
    :param x_kubernetes_list_type: x-kubernetes-list-type annotates an array to further
        ...
        3 possible values:
            ...
            #) `map`: These lists are like maps in that their elements have a \
            non-index key used to identify them. Order is preserved upon merge. The \
    ...
    :param x_kubernetes_map_type: x-kubernetes-map-type annotates an object to further
        ...
        and may have 2 possible values:

            #) `granular`: These maps are actual maps (key-value pairs) and each \
            fields are independent from each other (they can each be manipulated by \
            separate actors). This is the default behaviour for all maps.
            #) `atomic`: the list is treated as a single entity, like a scalar. \
