from avionix.kube.authentication import TokenReview, TokenReviewSpec, TokenRequest, TokenRequestSpec
...
@pytest.mark.xfail(
    raises=HelmError, reason="Helm does not support token request"
)
def test_token_request(chart_info):
    builder = ChartBuilder(chart_info, [TokenRequest(ObjectMeta(
        name="token-request"), TokenRequestSpec(["audience1", "audience2"], None, None)
    )])
    with ChartInstallationContext(builder):
