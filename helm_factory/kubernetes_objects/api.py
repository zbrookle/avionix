...
from helm_factory.kubernetes_objects.service import ServiceReference
...
class APIServiceSpec(HelmYaml):
    """
    :param ca_bundle: Group is the API group name this server hosts
    :param group: GroupPriorityMininum is the priority this group should have at \
        ...
        priority. The primary sort is based on GroupPriorityMinimum, ordered highest \
        number to lowest (20 before 10). The secondary sort is based on the \
    ...
    :param group_priority_minimum: InsecureSkipTLSVerify disables TLS certificate \
        verification when communicating with this server. This is strongly \
        discouraged.  You should use the CABundle instead.
    :param insecure_skip_tlsverify: Service is a reference to the service for this API \
        server.  It must communicate on port 443 If the Service is nil, that means the \
        handling for the API groupversion is handled locally on this server. The call \
        will simply delegate to the normal handler chain to be fulfilled.
    :param service: Version is the API version this server hosts.  For example, "v1"
    :param version: VersionPriority controls the ordering of this API version inside \
        of its group.  Must be greater than zero. The primary sort is based on \
...
class APIService(KubernetesBaseObject):
    """
    :param metadata: None
    :param spec: Spec contains information for locating and communicating with a \
        server
