...
from helm_factory.kubernetes_objects.container import Container, EphemeralContainer
from helm_factory.kubernetes_objects.metadata import ListMeta, ObjectMeta
from helm_factory.kubernetes_objects.node import NodeAffinity
from helm_factory.kubernetes_objects.reference import (
    CrossVersionObjectReference,
    LocalObjectReference,
)
from helm_factory.kubernetes_objects.security import (
    SELinuxOptions,
    WindowsSecurityContextOptions,
)
from helm_factory.kubernetes_objects.selector import LabelSelector
from helm_factory.kubernetes_objects.topology import TopologySpreadConstraint
from helm_factory.kubernetes_objects.volume import Volume
...
class PodSpec(HelmYaml):
    ...
    :param dns_config: Specifies the DNS parameters of a pod. Parameters specified \
    ...
    :param enable_service_links: EnableServiceLinks indicates whether information \
        about services should be injected into pod's environment variables, matching \
    ...
    :param host_network: Host networking requested for this pod. Use the host's \
        network namespace. If this option is set, the ports that will be used must be \
    ...
    :param priority_class_name: If specified, indicates the pod's priority. \
        ...
        there is no default.
    :param readiness_gates: If specified, all readiness gates will be evaluated for \
        pod readiness. A pod is ready when all its containers are ready AND all \
        conditions specified in the readiness gates have status equal to "True" More \
    ...
    :param termination_grace_period_seconds: Optional duration in seconds the pod \
        needs to terminate gracefully. May be decreased in delete request. Value must \
        be non-negative integer. The value zero indicates delete immediately. If this \
    ...
    :param topology_spread_constraints: TopologySpreadConstraints describes how a \
        group of pods ought to spread across topology domains. Scheduler will schedule \
        pods in a way which abides by the constraints. This field is only honored by \
