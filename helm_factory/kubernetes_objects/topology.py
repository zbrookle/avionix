...
class TopologySpreadConstraint(HelmYaml):
    """
    :param label_selector: LabelSelector is used to find matching pods. Pods that \
        match this label selector are counted to determine the number of pods in their \
    ...
    :param max_skew: MaxSkew describes the degree to which pods may be unevenly \
        ...
        not allowed.
    :param topology_key: TopologyKey is the key of node labels. Nodes that have a \
        label with this key and identical values are considered to be in the same \
        topology. We consider each <key, value> as a "bucket", and try to put balanced \
        number of pods into each bucket. It's a required field.
    :param when_unsatisfiable: WhenUnsatisfiable indicates how to deal with a pod if \
        it doesn't satisfy the spread constraint. - DoNotSchedule (default) tells the \
...
class TopologySelectorTerm(HelmYaml):
    """
    :param match_label_expressions: A list of topology selector requirements by \
        labels.
    """
