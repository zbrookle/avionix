...
class WebhookClientConfig(HelmYaml):
    """
    :param ca_bundle: service is a reference to the service for this webhook. Either \
        service or url must be specified.  If the webhook is running within the \
        cluster, then you should use `service`.
    :param url: None
    :param service: url gives the location of the webhook, in standard URL form \
    ...
    def __init__(
        self, ca_bundle: str, url: str, service: Optional[ServiceReference] = None
    ):
...
class RuleWithOperations(HelmYaml):
    ...
    :param api_versions: APIVersions is the API versions the resources belong to. \
        '\\*' is all versions. If '\\*' is present, the length of the slice must be \
        one. Required.
